...
def resnet(num_layers, num_classes, device):
    return ResNet_s(BasicBlock, [3, 3, 3, 3], num_layers, num_classes=num_classes, device=device)
...
def train_resnet_model(model:ResNet_s, dataloader:DataLoader, test_dataloader:DataLoader, epochs:int, learning_rate:float, device):
  ...
  for epoch in range(epochs):
    ...
    print(f"Epoch {epoch + 1} Validation Accuracy - Top-1: {top1_accuracy:.2f}%; Top-5 Accuracy: {top5_accuracy:.2f}%")

    if (top1_accuracy > best_accuracy):
      best_accuracy = top1_accuracy
      torch.save(model.state_dict(), "best_current_model.pth")
